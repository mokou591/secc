package cn.poi591.secc.constant;

public interface Path {
	public static final String JSP_USER = "/user";
	
	public static final String JSP_ACTIVITY = "/activity";
	public static final String JSP_ACTIVITY_NOTE = "/activity/note";
	
	public static final String JSP_FILM = "/film";
	public static final String JSP_FILM_REVIEW = "/film/review";
	
	public static final String JSP_BOOK = "/book";
	public static final String JSP_BOOK_REVIEW = "/book/review";
	
	public static final String JSP_MUSIC = "/music";
	public static final String JSP_MUSIC_REVIEW = "/music/review";

}
