					...
					<c:if test="${not empty userAuthority }">
						...
						<c:choose>
							<c:when test="${idHotnew == false}">
								<a class="btn btn-primary" href="${ctx}/admin/book/${mainBook.id}/set/hotnew">设置为新品/热门</a>
							</c:when>
							<c:when test="${idHotnew == true}">
								<a class="btn btn-danger" href="${ctx}/admin/book/${mainBook.id}/delete/hotnew">取消设置为新品/热门</a>
							</c:when>
						</c:choose>
					</c:if>
				</div>
