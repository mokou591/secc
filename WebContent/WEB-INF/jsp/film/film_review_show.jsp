					...
					<c:if test="${not empty userAuthority }">
						...
						<c:choose>
							<c:when test="${isEssence == false}">
								<a class="btn btn-primary" href="${ctx}/admin/film/${review.id}/setEssence/is">设置为精华</a>
							</c:when>
							<c:when test="${isEssence == true}">
								<a class="btn btn-danger" href="${ctx}/admin/film/${review.id}/setEssence/not">取消设置为精华</a>
							</c:when>
						</c:choose>
					</c:if>
				</div>
